...
export async function processPendingCarrotObservations(env: DatabaseEnv): Promise<ProcessorResult> {
  ...
  for (const observation of pending.results) {
    try {
      ...
      failed++;
      console.error(`Failed to process carrot observation ${observation.id}`, error);
    }
  }
...
async function createCandidate(
  ...
  if (!result.success || result.meta.last_row_id === undefined) {
    throw new Error("Carrot candidate insert failed.");
  }
...
async function assignObservation(
  ...
  await db.prepare(`
    INSERT OR IGNORE INTO carrot_candidate_members (
      candidate_id,
      observation_id,
      installation_hash
    ) VALUES (?, ?, ?)
